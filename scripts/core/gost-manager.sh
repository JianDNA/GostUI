...
deploy_system() {
    ...
    if command -v curl >/dev/null 2>&1 && command -v unzip >/dev/null 2>&1; then
        ...
        local zip_url="https://github.com/JianDNA/GostUI/archive/refs/heads/main.zip"
        local zip_file="GostUI-main.zip"

        if curl -L --progress-bar -o "$zip_file" "$zip_url" 2>/dev/null; then
            if [ -f "$zip_file" ] && [ -s "$zip_file" ]; then
                echo "✅ ZIP下载成功 ($(du -h "$zip_file" | cut -f1))"

                if unzip -q "$zip_file" 2>/dev/null; then
                    if [ -d "GostUI-main" ]; then
